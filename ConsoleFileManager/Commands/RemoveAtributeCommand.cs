...
{
    internal class RemoveAtributeCommand : ICommand
    {
        public void Execute(CommandArgumentsGetter commands, SerializeModel model, ConsolePrinter console, DirectoryInformation information)
        {
            ...
            if (file != null)
            {
                ...
                {
                    case "Hidden":
                        File.SetAttributes(file.FullName, attributes & (attributes ^ FileAttributes.Hidden));
                        break;
                    case "ReadOnly":
                        File.SetAttributes(file.FullName, attributes & (attributes ^ FileAttributes.ReadOnly));
                        break;
                    case "Archive":
                        File.SetAttributes(file.FullName, attributes & (attributes ^ FileAttributes.Archive));
                        break;
                    case "System":
                        File.SetAttributes(file.FullName, attributes & (attributes ^ FileAttributes.System));
                        break;
                    default:
